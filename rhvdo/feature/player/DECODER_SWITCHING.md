...
RHvdo uses nextlib's `DecoderManager` on one `ExoPlayer`. Video and audio choices are
...
RHvdo owns fallback policy. Each track keeps a bounded queue of fallback modes;
...
Tested RHvdo `fa76f296` against local nextlib `72b4125`, including nextlib's merge of
`origin/main` (`aafaf8a`). `assembleDebug`, `test`, and `ktlintCheck` passed with local composite
substitution and test failures enforced (158 RHvdo tests, 10 nextlib tests). The APK's
